...
struct hashTable
{
    ...
    unsigned int prim_hashFunction(string key)
    {
    ...
    unsigned int sec_hashFunction(string key)
    {
        unsigned int tmp = 0;
        for (int i = 0; i < key.length(); i++)
        {
        ...
        return 1 + ((tmp % m) % (capacity - 1));
    }

    void add(string key, int val)
    {
        unsigned int ind = prim_hashFunction(key);
        unsigned int ind1 = sec_hashFunction(key);

        for (int i = 0; i < capacity; i++)
        {
            unsigned int curInd = (ind + i * ind1) % capacity;
